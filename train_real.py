...
#         print(f"Weather-Aware Epoch {epoch}/50 -> Loss: {total_epoch_loss / steps:.5f} | Average Alpha: {alpha_tracker / steps:.4f}")
...
##----------------new code tha has been added to save the training metrics to a JSON file for later analysis and visualization-------------------##
import os
import json
import torch
import numpy as np
...
def execute_real_world_training():
    ...
    for epoch in range(1, 51):
        ...
        epoch_loss = total_epoch_loss / steps
        epoch_alpha = alpha_tracker / steps
        
        print(f"Weather-Aware Epoch {epoch}/50 -> Loss: {epoch_loss:.5f} | Average Alpha: {epoch_alpha:.4f}")
        
        metrics_log.append({
            "epoch": epoch,
            "loss": float(epoch_loss),
            "average_alpha": float(epoch_alpha)
        })
    ...
    print("Weather-aware checkpoint and training metrics saved successfully.")
